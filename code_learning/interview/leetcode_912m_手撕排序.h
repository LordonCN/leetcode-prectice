...
vector<int> quickSort(vector<int>&nums,int l,int r)
{
    ...
    if(first<last)
    {
        // 912题加随机选取参考value leetcode上可过 提速明显 可不加 记住是对区间长度进行取余
        int i = rand() % (r-l+1) + l;
        swap(nums[l],nums[i]);
...
void makeheapGreat(vector<int>&nums,int k,int length)
{
    for(int i = 2*k+1 ; i<length ; i = i*2+1)// 不仅包含交换 还有下坠处理 索引到节点的左孩子
    {
        ...
        k = i;// 更新孩子位置 交给循环去判断是否可下坠 继续查找
    }
    return ;
}

void sortHead(vector<int>&nums)
{
