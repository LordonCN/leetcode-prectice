...
#include "listNode.h"
using namespace std; 
/*结构体初始化的时候用*跟不用到底有什么区别? 使用new创建是另外开辟一个新的存储空间？*/
...
void ClientCode()
{
    singleListNode list_single;
    // init test
    if(initSingleList(&list_single))
    {
        cout<<"The list has been inited"<<endl;
        cout<<"the data is: "<<list_single.data<< endl;
        cout<<"the address is: "<<&list_single<< endl;
    }
