...
这一方面我们使用锁只是为了保证在下方控制指令的过程中不会被其他
指令所干扰，单个锁不存在互斥的情况，所以没有发生。

对于我们当时的使用环境，有两种方式能够避免死锁：
1、在不同线程上按相同的顺序获取多个锁，或者使用层级锁。
2、一个线程负责一部分任务，单个任务内部加锁保证不受干扰即可。

### 死锁
- 原因：死锁的的每个线程都互相的包含一个互斥量，且处于等待另一个解锁的状态。

因为他们都在等待对方先释放互斥量，导致没有线程能正常工作。
种情况就是死锁，它的问题就是由两个或两个以上的互斥量进行锁定,导致程序无法正常运行的情况。
...
2. 当线程内部需要`获取多个锁`，使用`std::lock(lhs.m,rhs.m)`对获取锁的操作上锁,
   配合std::adopt_lock交出权限，避免产生死锁。
